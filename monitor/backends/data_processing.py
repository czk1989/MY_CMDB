...
class DataHandler(object):
    ...
    def load_service_data_and_calulating(self,host_obj,trigger_obj,redis_obj):
        ...
        positive_expressions = [] #报警的时候，让用户知道，是哪些条件导致触发器成立了
        ...
        for expression in trigger_obj.triggerexpression_set.select_related().order_by('id'):
            ...
            single_expression_res = expression_process_obj.process() #单条表达式处理方法，返回的是个dict, {'calc_res':True,'expression_obj':object}
            if single_expression_res:
                calc_sub_res_list.append(single_expression_res) #把单条结果放入表达式结果列表
        ...
        if expression_res_string:
            trigger_res = eval(expression_res_string) #算整个Trigger表达式的结果
...
class ExpressionProcess(object):
    ...
    def __init__(self,main_ins,host_obj,expression_obj,specified_item=None):
        ...
        self.expression_obj = expression_obj #单条表达式
        self.main_ins = main_ins #DataHandler instance
    ...
    def load_data_from_redis(self):
        ...
        approximate_data_range = [json.loads(i.decode()) for i in data_range_raw] #存的依然是大概的数据量
    ...
    def process(self):
        """取出指定的时间周期的数据，按照指定的数据 处理方法 对数据进行处理"""
        data_list = self.load_data_from_redis() #已经按照用户的配置把数据 从redis里取出来了, 比如 最近5分钟,或10分钟的数据
        data_calc_func = getattr(self,'get_%s' % self.expression_obj.data_calc_func) #self.get_avg....
