...
class AssetServer(object):
    ...
    def fetch_assets(cls, request):
        ...
        try:
            if request.GET.get('add', None):
                response.data = AssetForm().as_table()
                return response
            ret = {}
            conditions = cls.assets_condition(request)
            asset_count = Asset.objects.filter(conditions).count()
            page_info = PageInfo(request.GET.get('pager', None), asset_count)
            asset_list = Asset.objects.filter(conditions).values(*cls.asset_values())[
    ...
    def put_assets(cls, request):
        ...
        try:
            ...
            for row_dict in update_list:
                ...
                try:
                    Asset.objects.filter(id=nid).update(**row_dict)
                except Exception as e:
    ...
    def post_assets(cls, request):
        ...
        obj=AssetForm(data)
        if obj.is_valid():
            Asset.objects.create(**obj.cleaned_data)
