...
import time
import hashlib
from MY_CMDB.settings import ASSET_AUTH_HEADER_NAME
from MY_CMDB.settings import ASSET_AUTH_KEY
from MY_CMDB.settings import ASSET_AUTH_TIME
...
def api_auth_method(request):
    ...
    print('utils/auth.py--25',limit_timestamp, timestamp)
    ...
    result = ha.hexdigest()
    print('utils/auth.py--31',result, encrypt)
    if encrypt != result:
        return False
