...
function nbDataList(requestUrl) {
    ...
    function addInitData() {
        $.ajax({
            ...
            success: function (response) {
                if (response.status) {
                    $("#modal_add_form > div").html(response.data);
                    //var test=$("#modal_add_form select option");
                    //test.each(function () {
                    //    $(this).val($(this).text())
                   // })
                } else {
     ...
     function addData() {
        ...
        $.ajax({
            url: requestUrl,
            type: 'POST',
            data: $('#modal_add_form').serialize(),
            traditional: true,
            success: function (response) {
                if (response.status) {
                    refreshData();
                    // console.log(response.status)
                    //$("#moadl_add_detail").html(response.data);
                } else {
                    // alert(response.message);
                    $("#modal_add_form > div").html(response.data);
                }
    ...
    function bindMenuFunction() {
        ...
        $("#do_add").click(function () {
            $.Show('#shade,#modal_add');
            var title=$(".menu > a.active").text()+'-添加';
            $('#pro_title').text(title);
            addInitData();

        });
        $('#do_add_confirm').click(function () {
            addData();
        });
    ...
    function initTableBody(startNum, list, tbConfig) {
        ...
        $.each(list, function (k1, row) {
            ...
            if ($("#menu_asset").hasClass("active")){

                var toDetail="<a href='/cmdb/asset/"+row['id']+"/'>"+row['id']+"";
                tagNum.innerHTML = toDetail;
                $(tr).append(tagNum);
            }else {
